...
void C_sobel( uint32_t w, uint32_t h, uint8_t* data ) {
  ...
  for(ih=1;ih<(int)h-1;ih++) {
    ...
    for(iw=1;iw<(int)w-1;iw++) {
      for(ii=0;ii<4;ii++) {
      	int x_derivative = 
      		((int)m[ih+1][iw-1][ii] + 2*(int)m[ih+1][iw][ii] + (int)m[ih+1][iw+1][ii]) -
      		((int)m_row_0[iw-1][ii] + 2*(int)m_row_0[iw][ii] + (int)m_row_0[iw+1][ii]);
      	int y_derivative =
      		((int)m_row_0[iw+1][ii] + 2*(int)m_row_1[iw+1][ii] + (int)m[ih+1][iw+1][ii]) -
      		((int)m_row_0[iw-1][ii] + 2*(int)m_row_1[iw-1][ii] + (int)m[ih+1][iw-1][ii]);
        m[ih][iw][ii] = (int) sqrt(pow(x_derivative, 2) + pow(y_derivative, 2));
      }
    }
  }
  free(m_row_0);
